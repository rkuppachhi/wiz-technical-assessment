...
# 3. NETWORK: PRIVATE SUBNETS & NAT GATEWAY (Requirement: Private EKS Subnets)
...
resource "aws_nat_gateway" "gw" {
  ...
  subnet_id     = "subnet-0c07814355cfccdae" # Your verified public subnet
}
...
resource "aws_route_table_association" "p1" { subnet_id = aws_subnet.private_1b.id; route_table_id = aws_route_table.private.id }
resource "aws_route_table_association" "p2" { subnet_id = aws_subnet.private_1c.id; route_table_id = aws_route_table.private.id }
...
resource "aws_eks_cluster" "wiz_cluster" {
  ...
  vpc_config { subnet_ids = [aws_subnet.private_1b.id, aws_subnet.private_1c.id] }
}
...
resource "aws_iam_role" "eks_cluster_role" {
  ...
  assume_role_policy = jsonencode({
    ...
    Statement = [{
      Action = "sts:AssumeRole"
      Effect = "Allow"
      Principal = { Service = "eks.amazonaws.com" } # FIXED
    }]
  })
}
resource "aws_iam_role_policy_attachment" "eks_policy" { policy_arn = "arn:aws:iam::aws:policy/AmazonEKSClusterPolicy"; role = aws_iam_role.eks_cluster_role.name }
...
resource "aws_iam_role_policy_attachment" "node_policy" { policy_arn = "arn:aws:iam::aws:policy/AmazonEKSWorkerNodePolicy"; role = aws_iam_role.eks_nodes_role.name }
resource "aws_iam_role_policy_attachment" "cni_policy" { policy_arn = "arn:aws:iam::aws:policy/AmazonEKS_CNI_Policy"; role = aws_iam_role.eks_nodes_role.name }
resource "aws_iam_role_policy_attachment" "ecr_policy" { policy_arn = "arn:aws:iam::aws:policy/AmazonEC2ContainerRegistryReadOnly"; role = aws_iam_role.eks_nodes_role.name }
